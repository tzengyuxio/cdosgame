...
- **價值**：清單 + 基本資料的高 CP 起點。
- **`catalog_id` 前綴規則（青衫存檔編號，內參用）**：本庫 `catalog_id` 欄即青衫為其保存的遊戲光碟／磁片檔所編的來源側索引（**非廠商正式型號、非本站 ID**）。字首為公司碼，後接該公司自身編號；無公司編號者以 `Z` + 自行流水號。空白＝該款無光碟／磁片檔（早期未保存、大補帖或網路下載，也可能尚未整理）。

  | 前綴 | 含意 | 後綴規則 |
  |---|---|---|
  | `3W` | 第三波磁片遊戲 | 接該公司編號；無／不明者 `Z`+自編（registry 見 `3WG…`／`3WZ…`） |
  | `HT` | 漢堂磁片遊戲 | 接該公司編號（registry 多為 `HTG…`） |
  | `JX` | 精訊磁片遊戲 | 接該公司編號；無／不明者 `Z`+自編（見 `JXD…`／`JXP…`／`JXZ…`） |
  | `RS` | 資策科技磁片遊戲 | 接該公司編號；無／不明者 `Z`+自編（見 `RS…`／`RSZ…`） |
  | `SS` | 大宇磁片遊戲 | 接該公司編號（見 `SSG…`／`SSA…`） |
  | `SWE` | 智冠 軟世界貴族版系列 磁片 | 接該公司編號 |
  | `SWT` | 智冠 軟世界珍藏版系列 磁片 | 接該公司編號 |
  | `SWZ` | 智冠其他磁片（如電腦休閒世界等） | 自行編號（**跨廠商，與智冠正式型號無關**——SWZ001=俠影記、SWZ002=Maxis 模擬螞蟻、SWZ003=Artdink A列車3） |
  | `TTN` | 天堂鳥磁片遊戲 | `Z`+自行編號（registry 見 `TTNZ…`） |
  | `Z` | 其他公司磁片遊戲 | 自行編號 |
  | `OCD` | 存放區1 光碟遊戲 | 自行編號 |
  | `SCD` | 存放區2 光碟遊戲 | 自行編號 |

  ⚠ **不能當台灣發行／官方通路佐證**：`SCD`/`OCD` 是青衫合輯光碟索引、`SWZ` 是青衫自編跨廠碼，均非廠商官方碼；發行歸屬需另有一手佐證（實例見 `docs/backlog/game-entry-review.md` 巫術系列、SWZ001 條）。另 chiuinan 內文常見的 `n001`／珍/貴等「智冠電腦休閒世界」品號屬另一套編碼（見 `docs/backlog/kudgame-done-log.md`）。
