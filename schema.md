...
| `year` | int 1970–2030 \| null | | **台灣發行年**（本站以台灣記憶為主）；純國產＝原版年＝台灣年。收集到 2024，2004 後為邊界外。年份「屬哪一種／準不準」見下兩欄 |
...
| `developer_region` | enum \| null | | 開發地：`TW HK CN MO JP US CA BE FR ES IT GB DE CH PL CZ HR KR SG IE SE AU IL HU SU RU` |
...
| `size` | string \| null | | 發行媒體容量，**數量在前**（非硬碟安裝量；chiuinan 帶入的純數字 MB 已一律 null）。光碟 `1CD`／`2CD`／`1DVD`；磁片 `片數×單片容量`（`4×360K`、`1×360K`、`3×1.2M`，容量本身即規格辨識）；錄音帶 `1TAPE`／`2TAPE`（Apple II 卡帶，記捲數）。同一套件內多媒體以 `+` 連接（`1CD+3×1.2M`）；**同款多包裝**（如磁片版與光碟版並存）以 `／` 分列，順序對齊 `editions`（`4×360K／1CD`＝磁片版／光碟版）|
