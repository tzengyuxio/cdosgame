...
> **姊妹專案 TOCR 引用了本文**：`~/repos/tocr` 的 `docs/data-conventions.md`「遊戲與標籤的命名」一節，把本文的〈標題定名〉〈一款多譯名〉〈同名消歧義〉〈重複處理〉改寫成它自己資料模型的版本（它處理同一批 1990 年代台灣遊戲，但實體是雜誌目錄裡的遊戲提及，沒有 id-registry、沒有 aliases 欄位）。
>
> **本文是正本，兩邊要定期同步**：這裡改了決策鏈或判準，記得同步過去；那邊踩到的新情況也值得回饋回來。最後對照：2026-08-17。
...
## 一款多譯名：`title_zh` 主名怎麼選

同一款遊戲在台灣有**兩個以上正式譯名**時（常見於多家業者各自引進外文作品，如 Alley Cat 有第三波《竹籬笆外的春天》與亞洲軟體《頑皮貓》），`title_zh` 只能挑一個，其餘一律進 `title_aliases`。

**決定順序：**

1. **有台灣一手足跡者優先。** 一手＝實體包裝／磁片、當年雜誌報導或廣告、廠商型錄。**不計簡中來源的通行度**——這是「台灣產品導向」原則的延伸；`顽皮猫` 這類簡體形只進 aliases，不參與裁決。線上二手站台的頁面數量不是主判準（冷門遊戲的線上聲量低，不代表譯名不正當）。
2. **打平時，取與並列原文名對應得上的譯名。** 本庫外文款標題格式為 `中文名（原文名）`，並列的原文名要有解釋力；選一個與原文名毫無語意關聯的中文名，等於讓括號白放。例：`頑皮貓（Alley Cat）` 勝過 `竹籬笆外的春天（Alley Cat）`——後者是借同名電影的行銷式命名，與原名脫鉤。
3. **仍打平 → 取有實體佐證者**（包裝／掃描 > 型錄條目 > 線上轉載）。
4. **仍分不開 → 取較早引進者。** 放在最後而非最前，是因為早期產品包裝多半沒印發行日期，**引進時序通常不可考**，不能當主判準。

**配套：**

- 未獲選的譯名**全部**進 `title_aliases`，保搜尋與連結可達。
- 各家發行資料不必擇一：`publisher_tw` 與 `release_codes[]` 都是多值，並列即可。
- **判準要留痕跡**：選定的名應在正文或 `references` 看得出憑什麼（引哪份型錄、哪張包裝）；不留痕跡的話，下一個人只會再吵一次。
- 不採「發行商優先序」（如精訊 > 軟體世界 > 第三波…）：條目主體是遊戲而非某家公司的商品，位階表是主觀的、需人工維護，且會被新資料翻案。

## 標題定名（`title_zh` 用哪個寫法）

同一款遊戲在標題畫面、包裝、磁片標籤、雜誌上寫法不一時：

1. **以遊戲標題畫面為正名依據**（畫面 > 包裝 > 磁片／標籤 > 報導）。畫面是產品本體，不受經銷、再版與外部著錄影響。
2. **「台／臺」等同字異體並存時一律用「台」**。這類差異屬字形變體、不是不同名稱，且全庫既有條目一致用「台」（標準台灣麻將、超級台灣麻將、台灣麻將王2…），跟著畫面寫「臺」只會製造孤例並拖累搜尋。
3. **其餘未採用的寫法一律進 `title_aliases`**（含英文名、磁片標籤名、異體字版本），保搜尋與連結可達。

例：cdg-0006 標題畫面作「臺灣麻將」（v2.1 另有 TAIWAN MAHJONG）、磁片標籤作「16張台灣麻將」→ 定名 `台灣麻將（陳精誠）`，其餘寫法全入 `title_aliases`。

## 同名消歧義（title 後綴）

兩款（含以上）`title_zh` 相同、確為**不同作品**時，於標題尾端加**全形括號**後綴區分。原則：**辨識度高、最容易聯想的維度優先**——而撞名的「原因」通常就決定了哪個維度最好認，故用決策鏈而非固定單一順序（決策脈絡見 ADR-004）。

**決定順序：**

1. **外文遊戲、且有廣為人知的英文原名 → 用英文原名。** 這類款玩家/資料庫本就以英文名索引，辨識度最高。例：`七寶奇謀（The Goonies）`、`決戰西洋棋（Battle Chess）`、`屠龍戰記（Dragon Odyssey）`。
2. **否則 → 用年份（西元四位）。** 預設主軸；不同世代/重製/不同作最直觀，本庫國產為主、多數衝突年份即可分。例：`古墓奇兵（1996）`／`古墓奇兵（2013）`、`封神演義（1993）`／`封神演義（1998）`。
3. **年份相同或不詳、仍分不開 → 用發行商／代理（中文簡稱）。** 專治「同一款、不同台灣代理」。例：`銀河英雄傳說IV EX（微波）`／`銀河英雄傳說IV EX（大宇）`。
4. **仍分不開（同年同商）→ 補平台或地區／副標。** 例：`（日）`、`（DOS）`。
5. **早期個人作者掛帥的作品 → 用作者名。** 1980 年代中後期的本地小品多由個人具名撰寫、統一交同一家發行（如第三波），年份又常只能推定，此時發行商與年份都分不開，作者才是最好認的維度。例：`台灣麻將（陳精誠）`／`台灣麻將（黃文塘）`（兩款皆第三波發行、1986–87 年間，年份無法確定）。

**配套機制：**

- **撞名時全部都加後綴**，不留裸名給任一方（避免「誰是正主」之爭）；**裸名一律加入 `title_aliases`**，保搜尋/連結可達。
- 格式：全形括號（）。年份＝西元四位；發行商＝中文簡稱；英文名＝照原樣大小寫。
- **消歧義 ≠ 版本命名**：貴族版／珍藏版／重製版等「同一發行脈絡的不同版本」走版本命名慣例（歸 `editions[]` 或各自條目的版本標註），**不套消歧義後綴**。兩者最常被搞混。
- 超過兩款、單一維度不夠 → 組合（如年份＋發行商），但保持最短。
- 注意：`中文名（English）` 常只是外文款的**識別性附註**（本就掛英文原名，與有無撞名無關），那不是消歧義；只有真的撞名才依上面決策鏈選維度。
