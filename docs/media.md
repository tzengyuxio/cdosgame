...
| `ad` | 廣告 | 廣告 | 雜誌廣告等（廠商**付費**宣傳，與下方編輯報導 `press` 區別）|
| `press` | 報導 | 報導 | 雜誌等媒體對遊戲的**編輯報導**：發售前介紹、發售後評論、攻略、開發團隊訪談等。**刊名＋期號＋子類一併寫在 `caption`**（如「電腦遊戲世界 第124期・產業巡禮」）；`source` 用**數位來源** code（`scan`／`boneash`…，指誰數位化的，非刊物本身），不把雜誌放 `source`（否則 registry 會隨期號暴增）。多張用 `press-01…` |
| `title` | 標題畫面 | 截圖 | |
...
media: z.array(z.object({
  ...
  kind: z.enum([
    ...
    "manual-cover","manual","ad","press","title","screenshot","map","credits","other",
  ]),
...
2. **底部「媒體」圖庫（預設、主力）**：正文下方一個 section，縮圖 grid；點縮圖開 **lightbox** 看全圖。**未被特別擺位的圖一律自動落到這裡**，零維護。
   - **分組方案**：**遊戲頁用 4 組粗分類** `GAME_CATEGORIES`——①包裝實體（盒裝/附件/光碟磁片/說明書）②遊戲畫面（標題/截圖）③宣傳與報導（廣告/海報/報導/地圖/製作名單）④其他——避免一款圖橫跨多細類、每類只一兩張造成頁面冗長空洞；**公司／人物頁維持細分類** `CATEGORIES`（標誌/產品/照片各自成組，語意乾淨）。兩者皆只渲染有圖的組。
