...
- **⚠ 新來源要「融會」進既有正文，不是各自加一段**（使用者 2026-07-22 明確糾正）：補條目時每多一份掃描／報導，就在文末黏一段「據某報導…」把該來源換句話說搬一次——這是最常犯的錯。正確做法是**把新事實併進主題相符的既有段落**，全篇重讀一次、消掉重複（曾出現同一件「協和代理發行」在三段各講一次）。文章要讀起來像一篇文章，不是來源清單。
- **不寫「報導載明設計公司為X、發行公司為Y」這類轉引句**：那些資訊開頭段已經有了，複述等於廢話；規格細節（硬體需求、記憶體、光碟機倍速、操作介面用滑鼠或鍵盤）一律不入正文。有價值的是**只有該來源才有的內容**（玩法細節、開發過程、角色設定、上市時程異動）。
...
- **正文是百科內文、不是撰寫紀錄**——以下四類務必清掉（來源仍記 frontmatter `references`/`provenance`，正文只陳述事實）：
  ...
  2. **刪編輯備忘**：待考／待補／仍待後續補充／暫保留差異待考／重複條目待合併／需分開著錄／補基本 metadata 等整句移除，收在最後一句可查證事實（別用「待補」收尾）。**同樣禁「考據過程／修訂紀錄」敘述**——如「本條目原因名稱訛誤而長期以 unverified 保留，後經某廣告證實…據以更正」這類描述資料**如何被查證或修訂**的元敘述（使用者 2026-07 明確糾正，見 memory `no-editorial-notes-in-body`）：新證據直接寫成事實＋掛 `<sup class="cite">`，「為何改／原本存疑」記進 commit message 或 backlog，不進正文。
...
- **`## 版本` 區塊（同一作的多個版本先集中記在本體條目）**：資料片、中英文改版、重製版、移植版、不同公司重出的版本…**資料還不多時一律先寫進本體條目的 `## 版本`**，不急著發新 id。等某個版本累積到足夠獨立成篇，再拆出去、把該列改成 cross-link。
  ...
  - 範例（cdg-0621 水滸傳～天命之誓）：
    ...
    - **1989 原作**：光榮於 PC-9801 等日本機種推出的初版，DOS 中文版由…
    - **1998 Windows 95 重製版**：光榮「舊瓶新裝」系列第二作，[第三波](/companies/第三波)代理，改採滑鼠操作、配樂重編<sup class="cite" data-ref="cgw169"></sup>。
    ```
...
- **註釋**（`footnotes: ["純文字補充…"]`）＝**無連結**的說明，也可被 `[N]` 引用（編號接在 references 之後 `[k+1…]`）。keyed footnote 的 `key` 命名：雜誌來源用 `<刊名縮寫><期號>`（如 `swm70`、`ace96`），其餘一次性註腳用 `fn01`…，規則與刊名對照見 `docs/refs-convention.md`「footnote key 命名」。
