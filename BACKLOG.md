...
- [ ] **考據李偉傑在骨灰集散地 Facebook 社團的發文**（2026-07-29）：該作者持續發表 1980–90 年代台灣 DOS／Apple II 軟體的一手回顧，含存世品實物、產品編號與作者姓名等庫內查不到的線索。已用過一篇（〈[骨灰分享] 麻將 (IBM-PC Mahjong 澳大利亞 1986 Procon Software)〉2025-11-30，據以補出第三波 1986–87 麻將群作的作者、發行商與編號 GP-G01／62026，見 cdg-0006／3168／4038／5346）。**下一篇待讀**：<https://www.facebook.com/groups/1659118727741582/posts/4246005235719572/>；其餘發文亦應通盤掃過、逐篇萃取可入庫的事實。註：FB 社團貼文 WebFetch 抓不到，需人工貼文內容進來。
...
- [ ] **廠商↔遊戲↔系列 對照校訂（使用者 2026-06-18 指出有不少錯誤，未急於處理）**：`developer`/`publisher_tw` 字串歸屬、`series` 前綴歸組（如三國志過度聚合外傳、成吉思汗併蒼狼與白鹿）等對照關係需人工複核校正；屬資料層校訂，與站台呈現機制無關
  - **`developer` 欄被填成「發行商」是系統性病灶**（2026-08-01 補證）：補寫電腦玩家 105 期那批空白 stub 時，6 則裡有 4 則的 `developer` 是錯的，且錯法一致——填了原發行商或台灣發行商：cdg-1491 美麗新國度填 JoWooD（實為 neo Software）、cdg-2252 跳棋小精靈填智冠（實為晟業）、cdg-1287 亞特蘭提斯2 填 Cryo（正名 Cryo Interactive）、cdg-1453 美國空戰1918 填含糊的 Aqua（實為 Aqua Pacific，原發行商是 Project Two）。推測與 chiuinan 慣以「某某出品」記發行商有關，因此**凡 provenance 含 chiuinan 且 `publisher_tw` 為空的條目都是高風險群**，值得排一次全庫掃描：把 developer 值比對已知發行商名單（智冠、松崗、第三波、JoWooD、Cryo、GT Interactive…），命中者逐筆複查。順帶一提，同批也補了三個 `REGIONS` 缺口（SI／AT／BG），外國小廠條目補完時常撞到。
