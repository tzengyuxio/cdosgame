...
- [ ] **明訂 cite `<sup>` 與句號的相對位置**（2026-07-22）：全庫慣例是 `。<sup class="cite">`（句號在前）佔 86%（2980 : 496），但 `SKILL.md` 只寫「句末接」、`docs/refs-convention.md` 未著墨，導致補條目的 subagent 寫成句號在後、需事後 perl 統一。應在 `docs/refs-convention.md` 明訂**句號在前**並在 `game-entry` skill 呼應；順帶決定既有 496 處少數派要不要一併正規化。**等《遊戲世界》169／180 期的 OCR 補條目工作全部 commit 完再做**。
