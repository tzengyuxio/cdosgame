...
**手動新增款（來源整併層）**：在 `data/basematch-decisions.json` 加一筆 `"<候選名>": {"action":"append","fields":{…}}`，`fields` 給**新款**的初始 frontmatter 欄位（`title_aliases`/`year`/`developer`/`publisher_tw`/`genre`/`localization_level`/`license_status`/`release_codes`/`external_links`/`provenance`…），由 `merge_sources.apply_basematch()` 套用——用於「來源已有但被 fuzzy guard 擱在 review 佇列」或要自訂編號/外連的**新**款（如 cdg-4154 中國之心）。`action` 另有 `merge`（把候選併入既有 id）/`reject`。此檔是 base-match 比對決策，**不要拿來改既有款的欄位**。
